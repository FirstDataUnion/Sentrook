...
- Allow/Deny and `/sentrook pending` join OpenClaw's `plugin:` `/approve` id through the host approval-runtime client (the same path `/approve` uses). `plugin.approval.list` from the plugin runtime is often empty because those records are requester-bound. The plugin also listens for `plugin.approval.requested` and `requireApproval.onRegistered` when the host provides them, and retries the join shortly after `requireApproval` returns (the host mints the id after the hook).

- Unattended hosted reviews no longer call OpenClaw `requireApproval` (the host cancels those with “no approval-capable initiating surface”). Sentrook blocks with copy that shows the command, `/sentrook allowlist add <id>` (or the CLI equivalent), unattended sensitivity, and [openclaw#138853](https://github.com/openclaw/openclaw/issues/138853). `/sentrook allowlist add <id>` (and the timeline button) records the same local allowlist entry as Allow always. Live plugin cards for cron/heartbeat wait on that OpenClaw issue ([Sentrook#59](https://github.com/FirstDataUnion/Sentrook/issues/59)).
- Local allowlist `curl`/`wget` entries keep the host and path instead of collapsing to `curl <url>` (which used to refuse the record, or match every URL when flags were present). Pipes and `curl | bash` are still not stored.
...
- Operator log lines are joinable on their own: resolution/result copy `intent`, `intent_kind`, `unattended`, and `parent_session_id` (OpenClaw parent session key). `batch_size` is 1 for a single tool. Hosted `scan.log` is not copied. Result excerpts unwrap OpenClaw `{content:[{text}]}` envelopes. `effect` on results is `never_ran` for aborted/approval-unavailable, not always `ran`. Cancelled/timeout cards are `label_source: host` and are not posted to `/feedback`. `plugin_version` is baked into the gateway bundle.
