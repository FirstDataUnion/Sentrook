...
def main() -> int:
    ...
    for rule in rules:
        ...
        # gitleaks calls this `secretGroup`; we emit it as `capture_group`
        # because that is what it is — the *index* of the capture group holding
        # the credential, an integer like 1, never a credential itself. The old
        # name read as "the group containing the secret", which is close enough
        # to be misleading and is why a taint analyser treated the generator as
    ...
    # Writes detection *patterns*, not credentials — everything here comes from
    # the vendored MIT ruleset. A taint analyser previously read the emitted
    # `secretGroup` field as sensitive data reaching a file write; the field is
    # now `capture_group`, which is both accurate and no longer misleading.
    PY_OUT.write_text(
        json.dumps(
            {
    ...
    def _ts_entry(rule: dict) -> str:
        ...
        group = json.dumps(rule["captureGroup"]) if rule["captureGroup"] is not None else "null"
        return (
            f"  {{ id: {json.dumps(rule['id'])}, regex: {json.dumps(rule['regex'])}, "
            f"ignorecase: {'true' if rule['ignorecase'] else 'false'}, "
            f"entropy: {entropy}, captureGroup: {group} }}"
        )
