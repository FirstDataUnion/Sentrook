...
from sentrook.shadow.verify import (
    VerifyReport,
    format_verify_text,
    run_shadow_verify,
...
def test_verify_plugin_runtime_requires_before_tool_call():
    ...
    assert verify_plugin_runtime(runtime, plugin_id="sentrook-shadow") == []

    bad = dict(runtime)
    bad["typedHooks"] = [{"name": "after_tool_call"}]
    errors = verify_plugin_runtime(bad, plugin_id="sentrook-shadow")
    assert any("before_tool_call" in e for e in errors)


def test_run_shadow_verify_sidecar_only():
    health = {"status": "ok", "rules_loaded": 22, "corpus_examples": 10}
    with patch("sentrook.shadow.verify.fetch_sidecar_health", return_value=health):
        report = run_shadow_verify(url="http://127.0.0.1:9099")
    assert report.ok
    assert report.sidecar_ok
    assert report.plugin_ok is None


def test_run_shadow_verify_sidecar_failure():
    with patch(
        "sentrook.shadow.verify.fetch_sidecar_health",
...
def test_format_verify_text_pass():
    report = VerifyReport(
        sidecar_url="http://sentrook-shadow:9099",
        sidecar_ok=True,
        sidecar_health={
            "status": "ok",
            "rules_loaded": 22,
...
def test_run_shadow_verify_with_openclaw_plugin(tmp_path):
    ...
    with (
        patch("sentrook.shadow.verify.fetch_sidecar_health_via_compose", return_value=health),
        patch("sentrook.shadow.verify.gateway_fetch_health", return_value=True),
        patch(
            "sentrook.shadow.verify.inspect_openclaw_plugin_runtime",
        ...
        report = run_shadow_verify(
            ...
            sidecar_service="sentrook-shadow",
        )
...
def test_run_shadow_verify_missing_compose_project(tmp_path):
    health = {"status": "ok", "rules_loaded": 22, "corpus_examples": 10}
    with patch("sentrook.shadow.verify.fetch_sidecar_health_via_compose", return_value=health):
        report = run_shadow_verify(
            url="http://sentrook-shadow:9099",
